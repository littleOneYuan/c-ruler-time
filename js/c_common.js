...
/**
 * @description 用于将获取昨天的时间
 */
export const yesterday = () => {
  let _now = new Date
  _now=_now.getFullYear() + "-" + (_now.getMonth()> 9 ? (_now.getMonth() + 1) : "0" + (_now.getMonth() + 1)) + "-" +(_now.getDate()> 9 ? (_now.getDate()) : "0" + (_now.getDate()));
  let time=(new Date).getTime()-24*60*60*1000;
  let yester=new Date(time);
  yester=yester.getFullYear() + "-" + (yester.getMonth()> 9 ? (yester.getMonth() + 1) : "0" + (yester.getMonth() + 1)) + "-" +(yester.getDate()> 9 ? (yester.getDate()) : "0" + (yester.getDate()));
  return yester;
}
/**
...
/**
 * @description 用于获取当前日期以及三十天之前的日期yyyy-mm-dd格式
 */
 export const months_to_now2 = () => {
  var today = '' // 今天
  var one_month = '' // 30天前
  var res = []
